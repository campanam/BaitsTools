...
### GUI Installation  
The GUI requires the Ruby gem 'tk' (typically installed using `gem install tk`). The GUI requires BaitsTools Ruby files to be in your $PATH. Please note that the GUI has only been tested on Mac OSX and may not work well on other operating systems.    

For Mac OSX users, there is a script `osx_install.sh` that will automatically install the scripts and modify your $PATH variable as needed. It will also install the [Ruby Version Manager](https://rvm.io/) and the tk gem. To install using the script: 
...
2. If BaitsTools is running slowly, try using the Rubinius Ruby compiler (https://rubinius.com/). Rubinius can multithread across multiple processors, while the standard Ruby interpreter only uses one processor (despite the number of requested threads).  

3. If you need to enter a negative value into the command-line interface (e.g. a negative melting temperature), omit the space between the parameter and the value (e.g. `-q-30.0` rather than `-q -30.0`). Including the space will cause a Ruby parsing error.  
