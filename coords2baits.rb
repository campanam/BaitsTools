	...
	File.open($options.infile, 'r') do |coord|
		while line = coord.gets
			...
			if refhash.include?(chromosome)
				...
				seq = Fa_Seq.new(chromosome + "_" + seqst + "-" + seqend, false)
				seqst = seqst.to_i-1 #Correct for 0-based counting
				seqend = seqend.to_i-1 #Correct for 0-based counting
				seq.seq = refhash[chromosome].seq[seqst..seqend]
				regions.push(seq)
			else
